...
def register_test_account(sender, **kwargs):
    ...
    test_cleaner_account = Account.objects.filter(user_id=123456)
    test_manager_account = Account.objects.filter(user_id=233333)
    if not test_cleaner_account or not test_manager_account:
        ...
        if not test_cleaner_account:
            test_cleaner_account = Account(user_id=123456,
                                           phone_number='123456',
                                           password='123456',
                                           gender='M',
                                           account_type='C',
        ...
        if not test_manager_account:
            test_manager_account = Account(user_id=233333,
                                           phone_number='233333',
                                           password='123456',
                                           gender='F',
                                           account_type='M',
